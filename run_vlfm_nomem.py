...
def _select_next_target(
    ...
    for data in frontiers_with_data:
        ...
        if f.frontier_id not in score_cache:
            ...
            ########## 수정된 부분: 이미지-텍스트 직접 매칭 ##########
            # VLM 분석 (descriptors와 vlm_likelihood_score는 여전히 필요)
            ...
            if target_objects:
                ...
                for target_obj in target_objects:
                    # 실제로 수정한 부분
                    # itm_scores = app_state.itm.image_text_scores(data["rgb"], [target_obj])
                    itm_scores = app_state.itm.image_text_scores(data["rgb"], [f"Seems like there is a {target_obj} ahead."])
                    if itm_scores is not None and itm_scores.size > 0:
                        all_itm_scores.append(float(itm_scores.max()))
